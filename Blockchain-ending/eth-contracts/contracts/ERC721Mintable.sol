...
contract Ownable {
    ...
    event OwnershipTransfered(address addrs);
    ...
    modifier onlyOwner(){
        require(_owner == msg.sender, "the caller requires to be owner.");
        _;
    }
...
contract Pausable is Ownable {
    ...
    event Paused(address addrs);
    event Unpaused(address addrs);
    ...
    modifier whenNotPaused(){
        require(!_paused, "Caller is not contract owner");
        _;
    }
    ...
    function setPauseStatus(bool isPaused) external onlyOwner {
        _paused = isPaused;
        if(isPaused){
...
contract ERC165 {
    ...
    function _registerInterface(bytes4 interfaceId) internal {
        require(interfaceId != 0xffffffff, "interfaceId is invalid.");
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(msg.sender == _tokenOwner[tokenId], "Approver should be the owner.");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "Caller not authorized for transfer. ");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public {
        ...
        require(_checkOnERC721Received(from, to, tokenId, _data), "Transfer not received.");
    }
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == from, "Transfer should be initiated by the token owner. ");
